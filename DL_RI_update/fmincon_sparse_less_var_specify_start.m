function [r1,x] = fmincon_sparse_less_var_specify_start(K, cond_prob, prob_act,joint_prob,start)
...
options = optimoptions(@fmincon,'PlotFcn','optimplotfval','Algorithm','active-set','MaxFunctionEvaluations',1000000,'MaxIterations',100000);%,'StepTolerance',1e-14,'OptimalityTolerance',0.5*1e-3);
[r1 r2 x r3] = fmincon(@objfun,start,A,-small*ones(NIAS,1),[],[],lb,ub,@NIAC,options);
end

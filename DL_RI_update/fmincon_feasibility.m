function [r1,x] = fmincon_feasibility(K, cond_prob, prob_act,joint_prob)
...
ub = ones(100*K+K,1);
ub(100*K+1:100*K+K) = 100;

function f = objfun(x)
   f = 0; %zero objective function, since we only need feasibility
end % Compute function value at x
disp('In Progress...');
%options = optimoptions(@fmincon,'Algorithm','active-set','MaxFunctionEvaluations',10000,'MaxIterations',10000,'PlotFcn','optimplotconstrviolation');%,'StepTolerance',1e-14,'OptimalityTolerance',0.5*1e-3);
options = optimoptions(@fmincon,'PlotFcn','optimplotconstrviolation','MaxFunctionEvaluations',100000,'MaxIterations',100000)%,'StepTolerance',1e-14,'OptimalityTolerance',0.5*1e-3);
